...
class Optimizer(object):
    ...
    def __init__(self, dimensions, base_estimator="gp",
        ...
        allowed_acq_funcs = ["gp_hedge", "EI", "LCB", "PI", "EIps", "PIps", "EIM"]
    ...
    def _tell(self, x, y, fit=True):
        ...
        if (fit and self._n_initial_points <= 0 and
            ...
            self.preyi = est.predict(self.space.transform(self.Xi))
            ...
            for cand_acq_func in self.cand_acq_funcs_:
                if cand_acq_func == "EIM":
                    values = _gaussian_acquisition(
                        X=X, model=est, y_opt=np.min(self.preyi),
