...
\hspace{2em}We also increased the noise level further (see Appendix B for details), and the conclusions remained consistent.
...
Keeping the other settings unchanged, now we increase the dimension of the test function to 4 dimensions (the number of initial points and the total budget are increased to 40 and 400 respectively), and we get the following result:
 ...
 \begin{figure}[ht]
    ...
    \label{Fig6}
\end{figure}
From the 4D figures, we can draw conclusions that are basically the same as those of 2D. Like 4.3, we also noticed that after the dimension increased, \textbf{total\_M} began to show a certain competitiveness compared with the other two output methods, especially in the simplest test function, Fn1, which was significantly ahead of the other two output methods. The characteristic of \textbf{total\_M} is that it is a completely exploitation output method, although obs\_M also output the maximum value of prediction mean, but its output is limited to the observations selected by the acquisition function EI. As we mentioned earlier, EI is an exploratory algorithm, and the size of the search space increases dramatically when the dimension is increased to 4D, so that the benefits of exploration in a limited budget may be much lower than that of exploitation. However, there is also some instability in the output result of \textbf{total\_M} (for example, Fn18 and Fn22), and the final result is not significantly better than obs\_M in most test functions, so we still do not recommend using the \textbf{total\_M} output method under four-dimensional conditions.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{.74\linewidth}
        \centering
        \includegraphics[width=1\textwidth]{D:/outputway/pictures/noise2D/noise2D_L.png}
    \end{subfigure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{.74\linewidth}
        \centering
        \includegraphics[width=1\textwidth]{D:/outputway/pictures/noise4D/noise4D_M.png}
    \end{subfigure}
