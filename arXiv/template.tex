...
\end{table}
  ...
  \hspace{2em}The first two columns in the table are the loss rate of the output methods $obs, total$ relative to the global optimal value of each test function. The third columns is the relative loss rates of sequence $total$ and sequence $obs$ respectively. For 2D problems, it's evident that the that the optimization efficiency of the predictive $total$, when applied to functions F2, F10, F11, and F12, is notably subpar. Reflecting on Table 1 reveals a shared characteristic among these functions: their conditioning levels are notably elevated, surpassing the $10^6$ limit. These functions' respective standard deviations hold significant values, leading to high noise values. Excessive noise can easily obscure the original structural characteristics of the function, complicating the Gaussian process modeling and hindering its predictive value. By examining Table 2, it becomes apparent that the loss rate of $obs$ related to these four functions is considerably higher, indicating that typical Bayesian optimization struggles due to diminished modeling precision in such scenario. Therefore, it can be concluded that for noise-free problems, poor Gaussian process modeling precision renders the application of predictive output techniques intolerable. For simpler functions, judging from the results in the third column, the loss rate of $total$ relative to $obs$ is mostly above $10\%$. For more complex functions F10, F12, etc., the resultss of optimization become increasingly dissatisfying. Therefore, for noise-free problems, we should indeed directly use the observation-based Bayesian optimization result output method (The loss rate of $output3$ relative to $obs$ in all test functions is greater than 0). For 4D problems, the conclution remains same, however, after increasing the dimension, we find that the relative loss rate of these two output methods begins to decrease on many functions.This will be discussed later in the article.
...
\begin{figure}[H]
    ...
    \label{Fig3}
\end{figure}
\hspace{2em}Figure 3 illustrates the optimization results with medium noise levels. It can be seen that after the noise increases, for most test functions, the gap between the observant output method OBS and the predictive output method further increases. In general, the conclusions of choosing output methods are basically consistent with those of small noise.
...
\section{Large homogeneous noise experimental results}
\subsection{2-dimensional experiments}
\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{.32\linewidth}
